...
def export_company_report(database,path:Path,exporter="Quản trị viên",license_result=None):
    ...
    cols.insert(3,("employee_code","Mã nhân viên"))
    ...
    wincols[2:2]=[("assigned_user","Người sử dụng"),("employee_code","Mã nhân viên")]
    ...
    _sheet(wb,"BAN_QUYEN_WINDOWS",["Mã tài sản","Tên máy","Người dùng","Mã nhân viên","Windows Edition","Activation Status","License Type","License Channel","Partial Key","Permanent","Expiration","Ngày kiểm tra"],[[r.get(k) for k in ("asset_code","computer_name","assigned_user","employee_code","os_edition","windows_activation_status","windows_license_type","windows_license_channel","windows_partial_key","windows_permanent_activation","windows_expiration","audit_date_display")] for r in records],exporter)
    office=[]
    for r in records:
        import json
        ...
        if not items:items=[{}]
        for x in items:office.append([r.get("asset_code"),r.get("computer_name"),r.get("assigned_user"),r.get("employee_code"),x.get("product_name") or r.get("office_product_summary"),x.get("version"),x.get("activation_status") or r.get("office_activation_summary"),x.get("mechanism"),x.get("partial_key"),x.get("expiration")])
    _sheet(wb,"BAN_QUYEN_OFFICE",["Mã tài sản","Tên máy","Người dùng","Mã nhân viên","Sản phẩm","Phiên bản","Activation Status","License Type","Partial Key","Expiration"],office,exporter)
    ...
    with database.connect() as con:
        ...
        logs=[list(x) for x in con.execute("SELECT completed_at,result,from_change_seq,to_change_seq,record_count,error_code,message FROM sync_log ORDER BY id")]
    ...
    _sheet(wb,"NHAT_KY_DONG_BO",["Thời gian","Kết quả","Từ change_seq","Đến change_seq","Số bản ghi","Mã lỗi","Nội dung"],logs,exporter)
